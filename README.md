...
额度窗口默认使用 `rolling` 滚动周期：每个 API Key 都从自己的首次放行开始计算。设置 `cycle_mode: anchored` 后，窗口改为从指定的 `anchor_at` 起点划分固定边界；所有绑定同一订阅计划的 API Key 共享边界，但用量和额度余额仍各自独立。

在管理中心新建或编辑订阅计划时，选择「锚定日期」，并按东八区填写锚定起点。也可以通过管理 API 提交以下窗口配置：
...
```json
{
  ...
  "cycle_mode": "anchored",
  "anchor_at": "2026-09-14T00:00:00+08:00"
}
```

以上配置会创建从 `2026-09-14 00:00:00 +08:00` 起、每 7 天重置一次的 400 USD 额度窗口。将 5 个 API Key 绑定到同一计划后，每个 Key 都会在同一东八区周边界重置，并各自拥有 400 USD 的周额度。

- 锚定起点必须是带时区偏移的 RFC 3339 时间；管理中心固定按东八区读写和显示该时间。
- 锚定起点到达前，请求会被拒绝，并给出可重试的开始时间。
- 到达新的边界无需定时任务；下一笔请求会按锚定边界进入当前周期。
- 将现有窗口切换为锚定周期，或修改其锚定起点、周期长度，会清空该窗口当前周期的累计用量后重新计算。管理中心会在保存前提示受影响的 API Key。
